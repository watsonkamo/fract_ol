...
/*   Updated: 2024/06/13 23:25:46 by eshintan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	valid_arg(int argc, char **argv)
{
	if (argc == 2 && !ft_memcmp(argv[1], "mandelbrot", 10))
		return (1);
	if (argc == 4 && !ft_memcmp(argv[1], "julia", 5) && (is_double(argv[2]))
		&& (is_double(argv[3])))
		return (1);
	return (0);
}
...
int	main(int ac, char **av)
{
	...
	else if (ac == 4 && !ft_memcmp(av[1], "julia", 6))
	{
		...
		return (ft_putstr_fd("Usage: ./fractol [mandelbrot/julia] \
		[julia_re julia_im]\n", 2), 0);
}
